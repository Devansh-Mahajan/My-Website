...
- Der Winkel $\alpha$ wird von der positiven x-Achse aus **gegen den Uhrzeigersinn** (mathematisch positive) gemessen8888. Winkel $> 2\pi$ oder negative Winkel (im Uhrzeigersinn) sind möglich 9.
...
### 3.1 Sinusund Cosinusfunktion

- **Graph:** Beide Funktionen erzeugen die bekannte **Sinuskurve** (Sinusoid).
	- $\sin(x)$ started bei $(0,0)$.
	- $\cos(x)$ started bei $(0,1)$. Der Cosinusgraph ist gegenüber dem Sinusgraphen um $\pi/2$ nach links verschoben ($\cos(\alpha) = \sin(\alpha + \pi/2)$).
- **Wertebereich:** $[-1, 1]$ für beide11.
- **Periodizität:** Beide sind periodisch mit der **Period $P = 2\pi$**12. D.h. $\sin(\alpha + 2\pi) = \sin(\alpha)$ und $\cos(\alpha + 2\pi) = \cos(\alpha)$.
...
### 3.2 Tangensund Cotangensfunktion
...
- **Periodizität:** Beide sind periodisch mit der **Period $P = \pi$**17. D.h. $\tan(\alpha + \pi) = \tan(\alpha)$ und $\cot(\alpha + \pi) = \cot(\alpha)$.
- **Verhalten für kleine Winkel $\alpha$ (im Bogenmaß):**
...
Herleitbar durch elementare Geometry am Einheitskreis (z.B. für $30^\circ, 45^\circ, 60^\circ$).
...
1. **Find eine Lösung $x_1$:** Mit der passenden Arcus-Funktion (z.B. $x_1 = \arcsin(c)$) 30. Dies liefert die Lösung im Hauptintervall.
2. **Find ggf. eine zweite Lösung $x_2$ im Basisintervall $[0, 2\pi[$:** Nutze Symmetrien am Einheitskreis oder Graphen 31.
	
	- Für $\sin(x)=c$: $x_2 = \pi - x_1$.
	- Für $\cos(x)=c$: $x_2 = 2\pi - x_1$ (oder $-\alpha$).
	- Für $\tan(x)=c$: Keine zweite Lösung im Interval der Länge $\pi$.
		
3. **Addiere alle Vielfachen der Period:** Um _alle_ Lösungen zu erhalten, addiere $k \cdot P$ (wobei $P$ die Period ist) zu den Basislösungen 32.
...
### 5.2 Quotientund Kehrwertbeziehungen
...
### 5.5 Doppel-, Halbeund Dreifachwinkelformeln
...
Die Verbindung zwischen den am Einheitskreis definierten Funktionen und der Geometry von Dreiecken.
...
Für beliebige Dreiecke gelten Sinusund Cosinussatz. Sie erweitern die Berechnungen auf nicht-rechtwinklige Fälle.
