[[206_skript_grenzwerte.pdf]]<br>
[[301_skript_ableitung.pdf]][[302_skript_ableitungen_skizzieren_und_berechnen.pdf]][[303_skript_graphenanalyse_und_optimierung.pdf]]<br>
[[304_skript_differentiationsregeln_I.pdf]]<br>
[[305_skript_differentiationsregeln_II.pdf]]

# 1. Folgen und Reihen

Dieses Kapitel behandelt die Grundlagen von Folgen und Reihen also geordnete Listen von Zahlen und deren Summen1111.
...
- **Definition (Formal):** Eine (reelle) Folge $a$ ist eine Function $a: \mathbb{N}_{>0} \rightarrow \mathbb{R}$, die jedem Index $n \in \mathbb{N}_{>0}$ eine reelle Zahl $a_n := a(n)$ zuordnet3.
- **Notation:** $(a_n)_{n\in\mathbb{N}>0}$ bezeichnet die Folge $a_1, a_2, a_3, …$4.
- **Darstellungsformen:**
	- **Explicit:** Eine direkte Formel für das $n$-te Glied, z.B. $a_n = 4n + 3$5555.
...
- **Arithmetische Folge:** Die Differenz $d$ zwischen aufeinanderfolgenden Gliedern ist constant: $d = a_{n+1} - a_n$12.
...
- **Allgemein:** Der Limes respektiert alle vier Grundrechenarten (Addition, Subtraktion, Multiplication, Division)28282828.
...
- **Differenzierbarkeit:** Eine Function heißt an der Stelle $x_0$ **differenzierbar**, wenn dieser Grenzwert existiert48.
...
- 1. Ableitung ($f'$): Die Function $f'$, die jeder Stelle $x$ ihre Ableitung $f'(x)$ zuordnet, heißt **Ableitungsfunktion**50505050.
- **2. Ableitung ($f''$):** Da $f'$ selbst eine Function ist, kann sie erneut abgeleitet werden. Dies ergibt die 2. Ableitung, $f'' = (f')'$51. Notation: $f''(x)$ oder $\frac{d^2f}{dx^2}$52.
...
- **Nullstellen von $f'$:** Wo $f$ ein **(locales) Minimum oder Maximum** hat (horizontal Tangente), hat $f'$ eine Nullstelle55555555.
- **Vorzeichen von $f'$ (Positive/Negative):**
	- Wo $f$ **steigt** (monoton wachsend), ist $f'$ **positive** (über der x-Achse)56565656.
	- Wo $f$ **fällt** (monoton fallend), ist $f'$ **negative** (under der x-Achse)57575757.
- **Extrema von $f'$:**
	- Wo $f$ am **steilsten** fällt (maximales Gefälle), hat $f'$ ein **locales Minimum**585858.
	- Wo $f$ am **steilsten** steigt (maximaler Anstieg), hat $f'$ ein **locales Maximum**59.
...
## 5.2. Potenz-, Exponentialund Trigonometrische Funktionen
...
## 5.3. Produktund Quotientenregel

8. **Produktregel:** $(f(x) \cdot g(x))' = f'(x)g(x) + f(x)g'(x)$
	
	- **Achtung:** Die Ableitung eines Produkts ist _nicht_ das Product der Ableitungen85858585.
...
- $g$ ist die **äußere Function**, $f$ ist die **innere Function**88.
- **Regel ("Außen mal Innen"):** $k'(x) = g'(f(x)) \cdot f'(x)$
	- Ableitung der äußeren Function (ausgewertet an der inneren Function) _mal_ der Ableitung der inneren Funktion89898989.
...
Ableitungen dienen also "Diagnoseinstrument" zur Analyse von Funktionen und zur Lösung von Optimierungsproblemen100.

## 6.1. Graphenanalyse (Kurvendiskussion)

Analyse der Kurvenform einer Function $f$ mittels $f'$, $f''$ und $f'''$101.
...
|Hinreichend: **Locales Minimum**||$f'(x_0) = 0$ **und** $f''(x_0) > 0$ 107107107107|
|Hinreichend: **Locales Maximum**||$f'(x_0) = 0$ **und** $f''(x_0) < 0$ 108108|
...
Ziel ist es, eine **Zielgrösse** (z.B. Fläche, Distanz) under bestimmten **Nebenbedingungen** (Einschränkungen) zu maximieren oder zu minimieren120120120120120120120120120.

### Vorgehensweise

1. **Zielgrösse** also Function aufstellen (oft mit mehreren Variable), z.B. $D(x,y) = x^2 + (y-1)^2$121121121.
2. **Nebenbedingung(en)** also Gleichung(en) formulieren, z.B. $y = x^2$122122122.
3. Nebenbedingungen in die Zielgrösse einsetzen, um eine Function in **einer Variable** zu erhalten, z.B. $D(x) = x^2 + (x^2-1)^2 = x^4 - x^2 + 1$123123123.
4. Extremstellen dieser Function finden:
...
- **Problem:** Find den Punkt auf der Parabel $y=x^2$, der dem Punkt (Planeten) $P(0,1)$ am nächsten ist128.
...
- **Analyse:**
	...
	- $D''(0) = -2 < 0 \Rightarrow$ Locales **Maximum** (!)135.
	- $D''(\pm \frac{\sqrt{2}}{2}) = 4 > 0 \Rightarrow$ (Absolutes) **Minimum**136.
- **Lösung:** Die dem Planeten am nächsten gelegenen Punkte sind $x = \pm \frac{\sqrt{2}}{2}$ (und $y = x^2 = 0.5$)137. Der Scheitelpunkt ($x=0$) ist ein locales _Maximum_ der Distanz138.
