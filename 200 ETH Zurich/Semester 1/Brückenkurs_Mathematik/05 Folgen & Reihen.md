...
Rätselfragen wie "Welche Zahl kommt also nächstes?" 3333sind oft Einführungen in das Konzept der Folgen4.
...
- **Formale Definition:** Eine (reelle) Folge $a$ ist eine Function $a: \mathbb{N}_{>0} \rightarrow \mathbb{R}$10. Jedem Index $n \in \mathbb{N}_{>0}$ wird eine reelle Zahl $a_n := a(n) \in \mathbb{R}$ zugeordnet11.

### 1.2. Folgen also Funktionen

Eine Folge ist eine spezielle Funktion12.

- Der Definitionsbereich (Input) einer Function $f(x)$ ist oft $\mathbb{R}$, während der Definitionsbereich einer Folge $a_n$ nur die natürlichen Zahlen $\mathbb{N}_{>0}$ (also 1, 2, 3, …) sind13.
- Der Graph einer Folge besteht daher nur aus einzelnen Punkten14. Diese Punkte liegen auf dem Graphen der entsprechenden reellen Funktion15. Man kann sagen, die Folge "tastet" die Function ab16.

**Beispiele:**

1. **Lineare Function (Bsp. g):**
	
	- Folge: $a_n = 4n + 3$ 17
	- Function: $f(x) = 4x + 3$ 18
		
2. **Exponentialfunktion (Bsp. f):**
	
	- Folge: $a_n = 2^{n-1}$ (oder $a_n = 0.5 \cdot 2^n$) 191919
	- Function: $f(x) = 0.5 \cdot 2^x$ 20
...
2. **"Hopse"-Spiel:**
	...
	- Explicit: $H_n = 0$ (gerade) 37, $H_n = 2^{(n-1)/2}$ (ungerade)38383838.
...
- Die $n$-te Partialsumme $s_n$ ist definiert also:
...
- Definition (Folge): Eine Folge heißt geometrisch, wenn der Quotient $q$ zweier aufeinanderfolgender Glieder constant ist59595959.

	$$

q = \frac{a_{n+1}}{a_n} = \text{constant}
